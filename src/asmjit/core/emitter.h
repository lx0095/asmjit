...
//! Validation options are used to tell emitters to perform strict validation of instructions passed to
//! \ref BaseEmitter::emit().
//!
...
//! by \ref ValidationOptions.
enum class ValidationOptions : uint32_t {
  ...
  //! created by Builder or Compiler.
  //!
  //! Default: false.
  kValidateIntermediate = 0x00000002u
};
ASMJIT_DEFINE_ENUM_FLAGS(ValidationOptions)
...
public:
  ...
  //! Validation options.
  ValidationOptions _validationOptions = ValidationOptions::kNone;
  ...
  //! Returns the emitter's validation options.
  inline ValidationOptions validationOptions() const noexcept { return _validationOptions; }

  //! Tests whether the given `option` is present in validation options.
  inline bool hasValidationOption(ValidationOptions option) const noexcept { return Support::test(_validationOptions, option); }

  //! Activates the given validation `options`.
  //!
  ...
  //! possibilities:
  //!
  //!   - Architecture specific assembler is used. In this case a \ref ValidationOptions::kValidateAssembler can be
  ...
  //!     \ref ValidationOptions::kValidateIntermediate option that adds explicit validation step before the Builder
  //!     or Compiler creates an \ref InstNode to represent an emitted instruction. Error will be returned if the
  //!     instruction is ill-formed. In addition, also \ref ValidationOptions::kValidateAssembler can be used, which
  ...
  ASMJIT_API void addValidationOptions(ValidationOptions options) noexcept;

  //! Deactivates the given validation `options`.
  //!
  //! See \ref addValidationOptions() and \ref ValidationOptions for more details.
  ASMJIT_API void clearValidationOptions(ValidationOptions options) noexcept;
