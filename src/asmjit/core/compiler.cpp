...
Error BaseCompiler::_newInvokeNode(InvokeNode** out, InstId instId, const Operand_& o0, const FuncSignature& signature) {
  InvokeNode* node;
...
Error BaseCompiler::_addInvokeNode(InvokeNode** out, InstId instId, const Operand_& o0, const FuncSignature& signature) {
...
Error BaseCompiler::_newConst(BaseMem* out, ConstPoolScope scope, const void* data, size_t size) {
  ...
  if (!_constPools[uint32_t(scope)])
    ASMJIT_PROPAGATE(_newConstPoolNode(&_constPools[uint32_t(scope)]));
