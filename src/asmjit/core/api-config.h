...
#define ASMJIT_LIBRARY_VERSION 0x020000 /* 2.0.0 */

#ifndef ASMJIT_ABI_NAMESPACE
//! AsmJit ABI namespace is an inline namespace within \ref asmjit namespace.
//!
//! It's used to make sure that when user links to an incompatible library, it won't link. It has also some additional
//! properties. When `ASMJIT_ABI_NAMESPACE` is defined by the user it would override the AsmJit default, which makes
//! it possible to use use multiple AsmJit libraries within a single project, totally controlled by the user of AsmJit.
#define ASMJIT_ABI_NAMESPACE _abi_2_0
#endif
